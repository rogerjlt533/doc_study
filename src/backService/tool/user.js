...
exports.calendarCount = async function (user_id, params) {
    ...
    if (common.empty(user_id) || common.empty(from_time) || common.empty(end_time)) {
        return 0
    }
    const note_type = common.empty(params.note_type) ? 1 : params.note_type
    const conditions = ['user_id=?', 'deleted_at is null'], options = [user_id]
    let sql = "select strftime('%Y-%m-%d',notes.created_at) as create_time, count(notes.id) as ts_count from notes where #CONDITION# group by create_time order by created_at"
    if (!common.empty(from_time)) {
        conditions.push("created_at>='" + from_time + " 00:00:00'")
    }
    if (!common.empty(end_time)) {
        conditions.push("created_at<='" + end_time + " 23:59:59'")
    }
